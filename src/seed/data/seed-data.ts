...
//     const roles: string[] = ['user', 'admin', 'super-admin'];
//     const generatedEmails: Set<string> = new Set();

//     for (let i: number = 1; i <= count; i++) {
//         let randomFixingUsers = Math.floor(Math.random() * 1000);
//         let randomRoles = Math.floor(Math.random() * roles.length);

//         // Generar un correo electrónico único
//         let email: string;
//         do {
//             email = `hola${randomFixingUsers}@gmail.com`;
//             randomFixingUsers++;
...
//             email,
//             password: 'Abc123',
//             fullName: `Hola${randomFixingUsers}`,
//             roles: [roles[randomRoles]]
//         });
//     }
//     return users;
// };
...
const generateUsers = (count: number) => {
    const users: SeedUser[] = [];
    const roles: string[] = ['user', 'admin', 'super-admin'];
    const emailBase: string = 'hola';

    for (let i: number = 1; i <= count; i++) {
        let email: string = `${emailBase}${i*2}@gmail.com`;
        let randomRoles: number = Math.floor(Math.random() * roles.length);

        users.push({
            email,
            password: bcryptPlugin.hasSync('Abc123'),
            fullName: `Hola${i}`,
            roles: [roles[randomRoles]]
        });
    }
